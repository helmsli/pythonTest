App.controller('initQuestionData', function($scope) {
	...
	{
		$scope.initQustionModel();
		$scope.initTemplatePath();
	}
	
	//初始化问题模型
	$scope.initQustionModel=function()
	{
		//项目定位
		var orientateArry=[];
		var i=0;
		var orientateScoreVal=[15,10,10,5];
		var subTotal="";
		while(i<4)
		{
			var orientate={"label":"","content":"","scoreVal":orientateScoreVal[i]};
			orientateArry.push(orientate);
			i++;
		}
		
		$scope.reportInfo.orientate.labels=orientateArry;
		$scope.reportInfo.orientate.subTotal=subTotal;
		//项目特征
		var featureLableArray=[];
		var j=0;
		var featureScoreVal=5;
		while(j<3)
		{
			var feature={"labels":[],"content":"","scoreVal":featureScoreVal,"subTotal":subTotal};
			var featureLabel={"label":""};
			featureLableArray.push(featureLabel);
			j++;
		}
		feature.labels=featureLableArray;
		$scope.reportInfo.feature=feature;
		
		//需求与市场提供的关系
		var demandLableArray=[];
		var k=0;
		var demandScoreVal=10;
		while(k<3)
		{
			var demand={"labels":[],"content":"","scoreVal":demandScoreVal,"subTotal":subTotal};
			var demandLabel={"label":""};
			demandLableArray.push(demandLabel);
			k++;
		}
		demand.labels=demandLableArray;
		$scope.reportInfo.demand=demand;
		
		//可行性
		var feasibilityArry=[];
		var h=0;
		var feasibilityScoreVal=[4,4,4,4,4,4];
		while(h<6)
		{
			var feasibility={"label":"","content":"","scoreVal":feasibilityScoreVal[h]};
			feasibilityArry.push(feasibility);
			h++;
		}
		$scope.reportInfo.feasibility.labels=feasibilityArry;
		$scope.reportInfo.feasibility.subTotal=subTotal;
		
		//总分值
		$scope.reportInfo.totalScoreVal=100;
	}
    ...
    $scope.initTemplatePath=function()
    {
		...
		var templateName=getExamineIndicesTemplatePath(projectName);
		//加载模板
   ...
   $scope.submitQuestion=function()
   {
    	...
    	//console.log(JSON.stringify(scope.reportInfo));
    	//return;
...
function initTableEvent()
{
	$("table").on("blur","td[contenteditable='true']",function(){
		...
		var groupName=$(this).parent().attr("data-name");
		var name=_this.attr("data-columnName");
		var value=_this.text();
		var scope=getAngularScope("initQuestionData");
		if((groupName=="demand"||groupName=="feature"))
		{
			if(name=="content" )
			{
				scope.reportInfo[groupName][name]=value;
			}else{
				if(name=="label")
				{
					scope.reportInfo[groupName]["labels"][index][name]=value;
				}
				if(name=="subTotal")
				{
					value=(value!="")?value:0;
					scope.reportInfo[groupName][name]=parseFloat(value);
				}
			}
		}else{
		...
		if(name=="subTotal")
		{
			var total1=(scope.reportInfo.feasibility.subTotal!="")?scope.reportInfo.feasibility.subTotal:0,
			total2=	(scope.reportInfo.demand.subTotal!="")?scope.reportInfo.demand.subTotal:0,
			total3=(scope.reportInfo.feature.subTotal!="")?scope.reportInfo.feature.subTotal:0,
			total4=(scope.reportInfo.orientate.subTotal!="")?scope.reportInfo.orientate.subTotal:0,
					total=total1+total2+total3+total4;
			scope.reportInfo.totalScore=total;
			$("#totalScore").text(scope.reportInfo.totalScore)
		}
...
{
	$.getJSON("/process/js/process_global/examine/eaxmineIndicesTemplate/eaxmineIndices_A.json",function(data){
		callBack(data);
	})
}
/**
