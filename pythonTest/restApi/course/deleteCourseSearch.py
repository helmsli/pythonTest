...
def queryTeacherInfo(baseUrl,teacherUserId):
	...
	log.debug(r.text)
	return response
def delTeacherInfo(baseUrl,willDelCourseId):
	website = '%s/courseSearch/deleteCourse' % (baseUrl)
	log.debug('del search course:' + website)
	willDelRequest={'courseId':willDelCourseId}
	log.debug(willDelRequest)
	r = requests.post(url=website,json=willDelRequest)   #带参数的GET请求
	log.debug(r.text)
	r.encoding = 'utf-8' #这里添加一行
	response = json_loads_byteified(r.text)
	#response=r.text
...
sys.setdefaultencoding('utf8')
	...
	#创建日志记录对象
log = Logger('delCourse.log');
log.debug(("start program ********************************************"));


serverUrl = 'http://www.chunzeacademy.com:8080'
teacherIdList=['1234578','5001000001']
for teacherId in teacherIdList:
	r=delTeacherInfo(serverUrl,teacherId)
	if r['retCode']==0:
		log.debug('del teacherInfo success:'+teacherId)
	else:
		log.debug('del teacherInfo error:'+teacherId)
	
print('finished del teacher')
